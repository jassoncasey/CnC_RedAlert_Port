...
// Header flags
constexpr uint16_t WWD_VQAHDF_AUDIO    = 0x0001;  // Has primary audio
constexpr uint16_t WWD_VQAHDF_ALTAUDIO = 0x0002;  // Has alternate audio

// Maximum values
constexpr int WWD_VQA_MAX_WIDTH  = 640;
constexpr int WWD_VQA_MAX_HEIGHT = 480;
...
constexpr uint16_t VQAHDF_AUDIO = WWD_VQAHDF_AUDIO;
constexpr int VQA_MAX_WIDTH = WWD_VQA_MAX_WIDTH;
constexpr int VQA_MAX_HEIGHT = WWD_VQA_MAX_HEIGHT;
...
public:
    ...
    // Prevent copying
    WwdVqaPlayer(const WwdVqaPlayer&) = delete;
    WwdVqaPlayer& operator=(const WwdVqaPlayer&) = delete;
    ...
    // Get playback state
    WwdVqaState GetState() const { return state_; }
    ...
    bool HasAudio() const { return (header_.flags & WWD_VQAHDF_AUDIO) != 0; }
    ...
    bool HasPreDecodedAudio() const {
        return fullAudioBuffer_ != nullptr && fullAudioSize_ > 0;
    }
...
// Frame callback for VQA_PlayWithCallback
...
// Playback with per-frame callback (blocking)
...
// Opaque handle (forward declared as class for C++ compatibility)
typedef class WwdVqaPlayer* WwdVqaHandle;

// Create/destroy
WwdVqaHandle Wwd_Vqa_Create(void);
void Wwd_Vqa_Destroy(WwdVqaHandle vqa);

// File operations
WwdBool Wwd_Vqa_LoadFile(WwdVqaHandle vqa, const char* filename);
WwdBool Wwd_Vqa_LoadMemory(WwdVqaHandle vqa, const void* data, uint32_t size);
void Wwd_Vqa_Unload(WwdVqaHandle vqa);
WwdBool Wwd_Vqa_IsLoaded(WwdVqaHandle vqa);

// Playback control
void Wwd_Vqa_Play(WwdVqaHandle vqa);
void Wwd_Vqa_Pause(WwdVqaHandle vqa);
void Wwd_Vqa_Stop(WwdVqaHandle vqa);
WwdBool Wwd_Vqa_NextFrame(WwdVqaHandle vqa);
WwdBool Wwd_Vqa_Update(WwdVqaHandle vqa, int elapsedMs);

// Frame access
int Wwd_Vqa_GetCurrentFrame(WwdVqaHandle vqa);
int Wwd_Vqa_GetFrameCount(WwdVqaHandle vqa);
int Wwd_Vqa_GetWidth(WwdVqaHandle vqa);
int Wwd_Vqa_GetHeight(WwdVqaHandle vqa);
int Wwd_Vqa_GetFPS(WwdVqaHandle vqa);
const uint8_t* Wwd_Vqa_GetFrameBuffer(WwdVqaHandle vqa);
const uint8_t* Wwd_Vqa_GetPalette(WwdVqaHandle vqa);

// Audio access
WwdBool Wwd_Vqa_HasAudio(WwdVqaHandle vqa);
int Wwd_Vqa_GetAudioSampleRate(WwdVqaHandle vqa);
int Wwd_Vqa_GetAudioSamples(WwdVqaHandle vqa, int16_t* buffer, int maxSamples);
